...
namespace NI2S.Node.Core
{
    ...
    public class EngineOptions
    {
        ...
        public EngineConfigurationLoader Configure(IConfiguration config, bool reloadOnChange)
        {
            ...
            var logger = EngineServices.GetRequiredService<ILogger<IEngine>>();
            var moduleLogger = EngineServices.GetRequiredService<ILogger<IEngineModuleManager>>();

            var loader = new EngineConfigurationLoader(this, config, hostEnvironment, reloadOnChange, logger, moduleLogger);
