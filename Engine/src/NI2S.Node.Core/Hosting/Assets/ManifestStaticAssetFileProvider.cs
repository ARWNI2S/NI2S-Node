...
internal sealed partial class ManifestStaticAssetFileProvider : IFileProvider
    {
        ...
        private readonly StaticAssetNode _root;

        public ManifestStaticAssetFileProvider(StaticAssetManifest manifest, Func<string, IFileProvider> fileProviderFactory)
        {
        ...
        public IDirectoryContents GetDirectoryContents(string subpath)
        {
            ...
            else
            {
                ...
                return new StaticAssetsDirectoryContents((files as IEnumerable<IFileInfo>) ?? Array.Empty<IFileInfo>());
            }

            HashSet<IFileInfo>? GetFilesForCandidatePatterns(string[] segments, StaticAssetNode? candidate, HashSet<IFileInfo>? files)
            {
            ...
            void GetCandidateFilesForNode(StaticAssetNode candidate, HashSet<IFileInfo> files)
            {
                foreach (var child in candidate.Children!)
                {
                    ...
                    {
                        // This is a folder
                        var file = new StaticAssetsDirectoryInfo(child.Key);
                    ...
                    else
                    {
                        ...
                        files.Add(string.Equals(child.Key, match.Path, _fsComparison) ? file :
                            ...
                            new StaticAssetsFileInfo(child.Key, file));
                    }
                }
            }
        }
        ...
        public IFileInfo GetFileInfo(string subpath)
        {
            ...
            StaticAssetNode? candidate = _root;
            List<StaticAssetPattern>? patterns = null;
            ...
            if (match != null)
            {
                ...
                {
                    return new StaticAssetsFileInfo(segments[^1], file);
                }
            }
        ...
        private sealed class StaticAssetsDirectoryContents : IDirectoryContents
        {
            private readonly IEnumerable<IFileInfo> _files;

            public StaticAssetsDirectoryContents(IEnumerable<IFileInfo> files) =>
                _files = files;
        ...
        private sealed class StaticAssetsDirectoryInfo : IFileInfo
        {
            private static readonly DateTimeOffset _lastModified = DateTimeOffset.FromUnixTimeSeconds(0);

            public StaticAssetsDirectoryInfo(string name)
            {
                Name = name;
            }
        ...
        private sealed class StaticAssetsFileInfo : IFileInfo
        {
            private readonly IFileInfo _source;

            public StaticAssetsFileInfo(string name, IFileInfo source)
            {
                Name = name;
                _source = source;
            }
        ...
        internal sealed class StaticAssetManifest
        {
            ...
            public StaticAssetNode Root { get; set; } = null!;

            internal static StaticAssetManifest Parse(Stream manifest)
            {
                return JsonSerializer.Deserialize(
                    manifest,
                    SourceGenerationContext.DefaultWithConverter.StaticAssetManifest)!;
            }
        }

        [JsonSourceGenerationOptions]
        [JsonSerializable(typeof(StaticAssetManifest))]
        [JsonSerializable(typeof(IDictionary<string, StaticAssetNode>))]
        ...
        internal sealed class StaticAssetNode
        {
            [JsonPropertyName("Asset")]
            public StaticAssetMatch? Match { get; set; }

            [JsonConverter(typeof(OSBasedCaseConverter))]
            public Dictionary<string, StaticAssetNode>? Children { get; set; }

            public StaticAssetPattern[]? Patterns { get; set; }
        ...
        internal sealed class StaticAssetMatch
        {
        ...
        internal sealed class StaticAssetPattern
        {
            [JsonPropertyName("ContentRootIndex")]
            public int ContentRoot { get; set; }
        ...
        private sealed class OSBasedCaseConverter : JsonConverter<Dictionary<string, StaticAssetNode>>
        {
            public override Dictionary<string, StaticAssetNode> Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
            {
                // Need to recursively deserialize `Dictionary<string, StaticAssetNode>` but can't deserialize
                ...
                var parsed = JsonSerializer.Deserialize(ref reader, SourceGenerationContext.DefaultWithConverter.IDictionaryStringStaticAssetNode)!;
                var result = new Dictionary<string, StaticAssetNode>(StaticAssetManifest.PathComparer);
                MergeChildren(parsed, result);
                return result;

                static void MergeChildren(
                    IDictionary<string, StaticAssetNode> newChildren,
                    IDictionary<string, StaticAssetNode> existing)
                {
                        ...
                        else
                        {
                            if (value.Patterns != null)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        var newList = new StaticAssetPattern[existingNode.Patterns.Length + value.Patterns.Length];
            ...
            public override void Write(Utf8JsonWriter writer, Dictionary<string, StaticAssetNode> value, JsonSerializerOptions options)
            {
