...
using NI2S.Node.Networking.Shared;
...
using System.Text;
using static NI2S.Node.Runtime.Message;

namespace NI2S.Node.Runtime.Messaging
{
    internal sealed class MessageSerializer
    {
            ...
            MessagingOptions options)
        {
            ...
            _requestContextCodec = NI2SGeneratedCodeHelper.GetService<DictionaryCodec<string, object>>(this, sessionPool.CodecProvider);
            _activationAddressCodec = NI2SGeneratedCodeHelper.GetService<IFieldCodec<GrainAddress>>(this, sessionPool.CodecProvider);
        ...
        private void ThrowInvalidHeaderLength(int headerLength) => throw new NI2SException($"Invalid header size: {headerLength} (max configured value is {_maxHeaderLength}, see {nameof(MessagingOptions.MaxMessageHeaderSize)})");
        private void ThrowInvalidBodyLength(int bodyLength) => throw new NI2SException($"Invalid body size: {bodyLength} (max configured value is {_maxBodyLength}, see {nameof(MessagingOptions.MaxMessageBodySize)})");
