...
namespace NI2S.Node.Infrastructure
{
    ...
    public partial class NI2SEngine : IEngine
    {
        ...
        protected virtual IServiceProvider GetServiceProvider(IServiceScope scope = null)
        {
            if (scope == null)
            {
                var accessor = ServiceProvider?.GetService<IDummyContextAccessor>();
                var context = accessor?.DummyContext;
                return context?.DummyServices ?? ServiceProvider;
            }
        ...
        public virtual void ConfigureRequestPipeline(INodeBuilder application)
        {
            ...
            foreach (var constructor in type.GetConstructors())
            {
                try
                {
                    ...
                    var parameters = constructor.GetParameters().Select(parameter =>
                    {
                        ...
                        if (service == null)
                            throw new NI2SException("Unknown dependency");
                        return service;
                    });
