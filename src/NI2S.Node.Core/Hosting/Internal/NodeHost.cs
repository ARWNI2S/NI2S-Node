...
namespace NI2S.Node.Hosting
{
    internal sealed partial class NodeHost : INodeHost, IAsyncDisposable
    {
        ...
        // TODO: Server as engine
        private IDummyServer Server { get; set; }
        ...
        public async Task StartAsync(CancellationToken cancellationToken = default)
        {
            ...
            var hostingApp = new DummyHostingApplication(application, _logger, diagnosticSource, activitySource, propagator, dummyContextFactory);
        ...
        private void EnsureStartup()
        {
            ...
            var startup = _hostingServiceProvider.GetService<IStartup>() ?? throw new InvalidOperationException($"No application configured. Please specify startup via INodeHostBuilder.UseStartup, INodeHostBuilder.Configure, injecting {nameof(IStartup)} or specifying the startup assembly via {nameof(NodeHostDefaults.StartupAssemblyKey)} in the web host configuration.");
            _startup = startup;
        }

        [MemberNotNull(nameof(Server))]
        private MessageDelegate BuildApplication()
        {
            Debug.Assert(_applicationServices != null, "Initialize must be called first.");
            ...
            try
            {
                ...
                var builderFactory = _applicationServices.GetRequiredService<INodeBuilderFactory>();
                var builder = builderFactory.CreateBuilder(Server.Features);
                builder.ApplicationServices = _applicationServices;

                var startupFilters = _applicationServices.GetService<IEnumerable<IStartupFilter>>();
                Action<INodeBuilder> configure = _startup!.Configure;
        ...
        private void EnsureServer()
        {
            ...
            if (Server == null)
            {
                Server = _applicationServices.GetRequiredService<IDummyServer>();
                ...
                if (addresses != null && !addresses.IsReadOnly && addresses.Count == 0)
                {
                    ...
                    //{
                    //    serverAddressesFeature!.PreferHostingUrls = StringUtilities.ParseBool(_config[NodeHostDefaults.PreferHostingUrlsKey]);
