...
namespace ARWNI2S.Node.Configuration
{
    ...
    public partial class SettingsHelper
    {
        ...
        public static NiisSettings SaveSettings(IList<IConfig> configurations, IEngineFileProvider fileProvider, bool overwrite = true)
        {
            ...
            var niisSettings = Singleton<NiisSettings>.Instance ?? new NiisSettings();
            niisSettings.Update(configurations);
            Singleton<NiisSettings>.Instance = niisSettings;
            ...
            var configuration = JsonConvert.DeserializeObject<NiisSettings>(fileProvider.ReadAllText(filePath, Encoding.UTF8))
                ?.Configuration
                ?? [];
