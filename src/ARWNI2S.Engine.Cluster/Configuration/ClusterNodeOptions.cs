...
namespace ARWNI2S.Cluster.Configuration
{
    ...
    public class ClusterNodeOptions
    {
        ...
        ///// Engines that enable this should validate an expected scheme is provided before using it.
        ///// </para>
        ///// </summary>
        ///// <remarks>
        ...
        /// </summary>
        public ClusterNodeLimits Limits { get; } = new ClusterNodeLimits();

        /// <summary>
        /// Provides a configuration source where endpoints will be loaded from on server start.
        /// The default is <see langword="null"/>.
        /// </summary>
        public ClusterConfigurationLoader ConfigurationLoader { get; set; }
        ...
        //        DevelopmentCertificate = GetDevelopmentCertificateFromNode(logger);
        //    }
        ...
        //private static X509Certificate2? GetDevelopmentCertificateFromNode(ILogger<KestrelServer> logger)
        //{
        //    try
        //    {
        //        var certs = CertificateManager.Instance.ListCertificates(NodeName.My, NodeLocation.CurrentUser, isValid: true, requireExportable: false);
        ...
        //            // a dotnet version prior to 7.0 - newer versions node it in such a way that this check succeeds.
        ...
        /// </summary>
        /// <returns>A <see cref="ClusterConfigurationLoader"/> for configuring endpoints.</returns>
        public ClusterConfigurationLoader Configure() => Configure(new ConfigurationBuilder().Build());
        ...
        /// <returns>A <see cref="ClusterConfigurationLoader"/> for further endpoint configuration.</returns>
        public ClusterConfigurationLoader Configure(IConfiguration config) => Configure(config, reloadOnChange: false);
        ...
        /// </param>
        /// <returns>A <see cref="ClusterConfigurationLoader"/> for further endpoint configuration.</returns>
        public ClusterConfigurationLoader Configure(IConfiguration config, bool reloadOnChange)
        {
            if (EngineServices is null)
            {
                throw new InvalidOperationException($"{nameof(EngineServices)} must not be null. This is normally set automatically via {nameof(IConfigureOptions<ClusterNodeOptions>)}.");
            }

            var configurationService = EngineServices.GetRequiredService<IClusterConfigurationService>();
            var certificatePathWatcher = reloadOnChange && !_disableCertificateFileWatching
                ...
                : null;
            var loader = new ClusterConfigurationLoader(this, config, configurationService, certificatePathWatcher, reloadOnChange);
