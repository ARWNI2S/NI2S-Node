        ...
        public virtual void LoadModuleInfo()
        {
            ...
            foreach (var item in GetDescriptionFilesAndDescriptors(modulesDirectory))
            {
                ...
                if (string.IsNullOrEmpty(moduleDescriptor.SystemName?.Trim()))
                    throw new ServerException($"A module '{descriptionFile}' has no system name. Try assigning the module a unique name and recompiling.");

                if (moduleDescriptors.Any(p => p.moduleDescriptor.Equals(moduleDescriptor)))
                    throw new ServerException($"A module with '{moduleDescriptor.SystemName}' system name is already defined");
                ...
                try
                {
                    ...
                    if (string.IsNullOrEmpty(moduleDirectory))
                        throw new ServerException($"Directory cannot be resolved for '{_fileProvider.GetFileName(descriptionFile)}' description file");
                ...
                catch (Exception exception)
                {
                    ...
                    throw new ServerException($"Module '{moduleDescriptor.FriendlyName}'. {exception.Message}", exception);
                }
            }
