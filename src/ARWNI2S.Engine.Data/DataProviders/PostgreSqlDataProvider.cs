...
namespace ARWNI2S.Engine.Data.DataProviders
{
    public partial class PostgreSqlServerDataProvider : BaseDataProvider, INiisDataProvider
    {
        ...
        protected virtual string GetSequenceName<TEntity>(DataConnection dataConnection) where TEntity : DataEntity
        {
            ...
            var descriptor = NI2SDataMappingSchema.GetEntityDescriptor(typeof(TEntity))
                             ?? throw new NI2SException($"Mapped entity descriptor is not found: {typeof(TEntity).Name}");
            ...
            if (string.IsNullOrEmpty(columnName))
                throw new NI2SException("A table's primary key does not have an identity constraint");
