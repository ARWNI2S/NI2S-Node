...
using ARWNI2S.Node.Data.DataProviders.LinqToDb;
...
{
    public partial class PostgreSqlServerDataProvider : BaseDataProvider, INiisDataProvider
    {
        #region Fields
        ...
        protected virtual string GetSequenceName<TEntity>(DataConnection dataConnection) where TEntity : DataEntity
        {
            ...
            var descriptor = DataMappingSchema.GetEntityDescriptor(typeof(TEntity))
                             ?? throw new NiisException($"Mapped entity descriptor is not found: {typeof(TEntity).Name}");
            ...
            if (string.IsNullOrEmpty(columnName))
                throw new NiisException("A table's primary key does not have an identity constraint");
        ...
        public virtual string BuildConnectionString(INodeConnectionStringInfo nopConnectionString)
        {
            ...
            if (nopConnectionString.IntegratedSecurity)
                throw new NiisException("Data provider supports connection only with login and password");
