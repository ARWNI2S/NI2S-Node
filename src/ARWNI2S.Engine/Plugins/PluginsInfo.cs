        ...
        public virtual void LoadPluginInfo()
        {
            ...
            foreach (var item in GetDescriptionFilesAndDescriptors(pluginsDirectory))
            {
                ...
                if (string.IsNullOrEmpty(pluginDescriptor.SystemName?.Trim()))
                    throw new NI2SException($"A plugin '{descriptionFile}' has no system name. Try assigning the plugin a unique name and recompiling.");

                if (pluginDescriptors.Any(p => p.pluginDescriptor.Equals(pluginDescriptor)))
                    throw new NI2SException($"A plugin with '{pluginDescriptor.SystemName}' system name is already defined");
                ...
                try
                {
                    ...
                    if (string.IsNullOrEmpty(pluginDirectory))
                        throw new NI2SException($"Directory cannot be resolved for '{_fileProvider.GetFileName(descriptionFile)}' description file");
