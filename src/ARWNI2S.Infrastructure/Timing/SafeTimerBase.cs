...
using ARWNI2S.Logging.Extensions;
...
namespace ARWNI2S.Timing
{
    ...
    internal class SafeTimerBase : IDisposable
    {
        ...
        private void Init(Func<object, Task> asynCallback, TimerCallback synCallback, object state, TimeSpan due, TimeSpan period)
        {
            ...
            logger.LogDebug(LogCode.TimerChanging, LocalizedStrings.TimerChanging_Creating, GetFullName(), due, period);
        ...
        public void DisposeTimer()
        {
            if (timer != null)
            {
                try
                {
                    var t = timer;
                    timer = null;
                    logger.LogDebug(LogCode.TimerDisposing, "Disposing timer {0}", GetFullName());
                    t.Dispose();

                }
                catch (Exception exc)
                {
                    logger.LogWarning(LogCode.TimerDisposeError,
        ...
        public bool CheckTimerFreeze(DateTime lastCheckTime, Func<string> callerName)
        {
            return CheckTimerDelay(previousTickTime, totalNumTicks,
                        dueTime, timerFrequency, logger, () => string.Format("{0}.{1}", GetFullName(), callerName()), LogCode.Timer_SafeTimerIsNotTicking, true);
        }

        public static bool CheckTimerDelay(DateTime previousTickTime, int totalNumTicks,
                        TimeSpan dueTime, TimeSpan timerFrequency, ILogger logger, Func<string> getName, LogCode code, bool freezeCheck)
        {
            ...
            var errMsg = string.Format(ErrorStrings.SafeTimerBase_CheckTimerDelay_ErrorMessage,
                ...
                previousTickTime.PrintDate(), // 2
                timeSinceLastTick,                  // 3
                exceptedTimeToNexTick);             // 4

            if (freezeCheck)
            {
                logger.LogError(code, errMsg);
            }
            else
            {
                logger.LogWarning(code, errMsg);
            }
            return false;
        }
        ...
        private bool Change(TimeSpan newDueTime, TimeSpan period)
        {
            ...
            logger.LogDebug(LogCode.TimerChanging, LocalizedStrings.TimerChanging, GetFullName(), newDueTime, period);
            ...
            {
                logger.LogWarning(LogCode.TimerChangeError,
        ...
        private void HandleSyncTimerCallback(object state)
        {
            try
            {
                logger.LogDebug(LogCode.TimerBeforeCallback, LocalizedStrings.TimerBeforeCallbackSync, GetFullName());
                syncCallbackFunc(state);
                logger.LogDebug(LogCode.TimerAfterCallback, LocalizedStrings.TimerAfterCallbackSync, GetFullName());
            }
            catch (Exception exc)
            {
                logger.LogWarning(LogCode.TimerCallbackError, string.Format(ErrorStrings.TimerCallbackErrorSync, exc.Message, GetFullName()), exc);
            }
            finally
            {
                previousTickTime = DateTime.UtcNow;
        ...
        private async void HandleAsyncTaskTimerCallback(object state)
        {
            ...
            try
            {
                logger.LogDebug(LogCode.TimerBeforeCallback, LocalizedStrings.TimerBeforeCallbackAsync, GetFullName());
                await asynTaskCallback(state);
                logger.LogDebug(LogCode.TimerAfterCallback, LocalizedStrings.TimerAfterCallbackAsync, GetFullName());
            }
            catch (Exception exc)
            {
                logger.LogWarning(LogCode.TimerCallbackError, string.Format(ErrorStrings.TimerCallbackErrorAsync, exc.Message, GetFullName()), exc);
            }
            finally
            {
                previousTickTime = DateTime.UtcNow;
        ...
        private void QueueNextTimerTick()
        {
            try
            {
                ...
                logger.LogDebug(LogCode.TimerChanging, LocalizedStrings.TimerChanging_Queue, GetFullName());
                ...
                if (timerFrequency == Constants.INFINITE_TIMESPAN)
                {
                    ...
                    logger.LogDebug(LogCode.TimerStopped, LocalizedStrings.TimerStopped, GetFullName());
                }
                else
                {
                    timer.Change(timerFrequency, Constants.INFINITE_TIMESPAN);

                    logger.LogDebug(LogCode.TimerNextTick, LocalizedStrings.TimerNextTick, GetFullName(), timerFrequency);
                }
            }
            ...
            {
                logger.LogError(LogCode.TimerQueueTickError,
