...
namespace ARWNI2S.Collections
{
    ...
    ///<seealso cref="Sorted.OrderedMultiDictionary&lt;TKey,TValue&gt;"/>
    [Serializable]
    internal class MultiDictionary<TKey, TValue> : MultiDictionaryBase<TKey, TValue>, ICloneable
    {
        ...
        public sealed override void Add(TKey key, TValue value)
        {
            ...
            if (_hash.Find(keyValues, false, out KeyAndValues existing))
            {
                ...
                if (!_allowDuplicateValues)
                {
                    ...
                    {
                        if (CollectionUtils.GetHashCode(existing.Values[i], _valueEqualityComparer) == valueHash &&
        ...
        public sealed override bool Contains(TKey key, TValue value)
        {
            ...
            if (_hash.Find(find, false, out KeyAndValues item))
            {
                ...
                for (int i = 0; i < existingCount; ++i)
                {
                    if (CollectionUtils.GetHashCode(item.Values[i], _valueEqualityComparer) == valueHash &&
        ...
        ///   b) We can't allow nodeClients to down-cast to the array and modify it.
        ...
        private IEnumerator<TValue> EnumerateValues(KeyAndValues keyAndValues)
        {
            ...
            int stamp = _hash.GetEnumerationStamp();
        ...
        {
            return new MultiDictionary<TKey, TValue>(_allowDuplicateValues, _keyEqualityComparer, _valueEqualityComparer, _equalityComparer,
