...
namespace ARWNI2S.Collections
{
    ...
    internal class RedBlackTree<T> : IEnumerable<T>
    {
        ...
        internal delegate int RangeTester(T item);
...
//            ConsoleText.WriteLine("{0}{1} {2,4} {3}", prefixNode, node.IsRed ? "@@" : "..", node.Count, node.item);
...
//            // Checks that this node is sorted with respect to any children.
//            if (node.left != null)
...
//            // Checks that the two-red rule is not violated.
//            if (node.IsRed)
//            {
//                if (node.left != null)
