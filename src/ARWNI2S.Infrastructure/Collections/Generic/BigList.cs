...
using ARWNI2S.Collections.Resources;
...
namespace ARWNI2S.Collections.Generic
{
    ...
    [Serializable]
    internal class BigList<T> : ListBase<T>, ICloneable
    {
        ...
        public BigList<T> CloneContents()
        {
            ...
            else
            {
                ...
                return new BigList<T>(CollectionAlgorithms.Convert(this, delegate (T item)
                {
                    if (item == null)
        ...
        private IEnumerator<T> GetEnumerator(int start, int maxItems)
        {
            ...
            if (root != null && maxItems > 0)
            {
                ...
                for (; ; )
                {
                    ...
                    {
                        yield return currentLeaf._items[i];
        ...
        private static Node NCopiesOfNode(int copies, Node node)
        {
            ...
            if (copies * (long)node._count > MAXITEMS)
                throw new InvalidOperationException(LocalizedStrings.Collections_CollectionTooLarge);
        ...
        /// <summary>
        /// Checks the balance of the current tree and rebalance it if it is more than BALANCEFACTOR
        ...
        [SuppressMessage("Microsoft.Design", "CA1061:DoNotHideBaseClassMethods", Justification = "This is appropriate to avoid generic conversions.")]
        public new BigList<TDest> ConvertAll<TDest>(Converter<T, TDest> converter)
        {
            return new BigList<TDest>(CollectionAlgorithms.Convert(this, converter));
        }
        ...
        public void Reverse()
        {
            CollectionAlgorithms.ReverseInPlace(this);
        }
        ...
        {
            CollectionAlgorithms.ReverseInPlace(Range(start, count));
        }
        ...
        public void Sort()
        {
            Sort(Compare.DefaultComparer<T>());
        }
        ...
        {
            CollectionAlgorithms.SortInPlace(this, comparer);
        }
        ...
        {
            Sort(Compare.ComparerFromComparison(comparison));
        }
        ...
        {
            return BinarySearch(item, Compare.DefaultComparer<T>());
        }
        ...
        {
            return BinarySearch(item, Compare.ComparerFromComparison(comparison));
        }
...
//        /// <summary>
//        /// Attempts to validate the public consistency of the tree.
//        /// </summary>
...
//        /// <summary>
//        /// Prints out the public structure of the tree, for debugging purposes.
//        /// </summary>
//        public void Print()
//        {
//            ConsoleText.WriteLine("SERIES: Count={0}", Count);
//            if (Count > 0)
//            {
//                Console.Write("ITEMS: ");
        ...
        private sealed class LeafNode : Node
        {
            ...
            private bool MergeLeafInPlace(Node other)
            {
                ...
                if (other is LeafNode otherLeaf && (newCount = otherLeaf.Count + _count) <= MAXLEAF)
                {
                    ...
                    Array.Copy(otherLeaf._items, 0, _items, _count, otherLeaf._count);
                    _count = newCount;
                    return true;
                }
                return false;
            ...
            /// the merged items. Does not modify 
            ...
            private Node MergeLeaf(Node other)
            {
                ...
                if (other is LeafNode otherLeaf && (newCount = otherLeaf.Count + _count) <= MAXLEAF)
                {
                    ...
                    T[] newItems = new T[MAXLEAF];
                    Array.Copy(_items, 0, newItems, 0, _count);
                    Array.Copy(otherLeaf._items, 0, newItems, _count, otherLeaf._count);
            ...
            public override Node PrependInPlace(T item)
            {
                ...
                if (_count < MAXLEAF)
                {
                    ...
                    {
                        T[] newItems = new T[MAXLEAF];
                        Array.Copy(_items, 0, newItems, 1, _count);
                        _items = newItems;
                    }
                    else
                    {
            ...
            public override Node InsertInPlace(int index, Node node, bool nodeIsUnused)
            {
                ...
                if (node is LeafNode otherLeaf && (newCount = otherLeaf.Count + _count) <= MAXLEAF)
                {
                    ...
                    if (newCount > _items.Length)
                    {
                        T[] newItems = new T[MAXLEAF];
                        Array.Copy(_items, 0, newItems, 0, index);
                        Array.Copy(otherLeaf._items, 0, newItems, index, otherLeaf.Count);
                        Array.Copy(_items, index, newItems, index + otherLeaf.Count, _count - index);
                        _items = newItems;
                    }
                    else
                    {
                        Array.Copy(_items, index, _items, index + otherLeaf.Count, _count - index);
                        Array.Copy(otherLeaf._items, 0, _items, index, otherLeaf._count);
                    }
                    _count = newCount;
                    return this;
                }
            ...
            public override Node RemoveRangeInPlace(int first, int last)
            {
                ...
                if (_count > last + 1)
                    Array.Copy(_items, last + 1, _items, first, _count - last - 1);
            ...
            public override Node RemoveRange(int first, int last)
            {
                ...
                if (first > 0)
                    Array.Copy(_items, 0, newItems, 0, first);
                if (_count > last + 1)
                    Array.Copy(_items, last + 1, newItems, first, _count - last - 1);
                return new LeafNode(newCount, newItems);
            }
            ...
            public override Node Subrange(int first, int last)
            {
                ...
                else
                {
                    ...
                    int n = last - first + 1;
                    T[] newItems = new T[n];
                    Array.Copy(_items, first, newItems, 0, n);
            ...
            {
                // Checking count and length of buffer.
                Debug.Assert(_count > 0);
                Debug.Assert(_items != null);
                Debug.Assert(_items.Length > 0);
                Debug.Assert(_count <= MAXLEAF);
                Debug.Assert(_items.Length <= MAXLEAF);
...
//            {
//                Console.Write("{0}LEAF {1} count={2}/{3} ", prefixNode, shared ? "S" : " ", _count, _items.Length);
...
//#endif //DEBUG
        ...
        private sealed class ConcatNode : Node
        {
            ...
            /// <param name="newLeft">New left child.</param>
            /// <param name="newRight">New right child.</param>
            /// <returns>Node with the given children. Usually, but not always,  Returns
            ...
            public override Node SetAtInPlace(int index, T item)
            {
                ...
                if (index < leftCount)
                {
                    Node newLeft = _left.SetAtInPlace(index, item);
                    if (newLeft != _left)
            ...
            public override Node PrependInPlace(T item)
            {
                ...
                if (_left.Count < MAXLEAF && !_left.Shared && _left is LeafNode leftLeaf)
                {
                    ...
                    if (c == leftLeaf._items.Length)
                    {
                        T[] newItems = new T[MAXLEAF];
                        Array.Copy(leftLeaf._items, 0, newItems, 1, c);
                        leftLeaf._items = newItems;
                    }
                    else
                    {
                        Array.Copy(leftLeaf._items, 0, leftLeaf._items, 1, c);
                    }

                    leftLeaf._items[0] = item;
                    leftLeaf._count += 1;
                    _count += 1;
                    return this;
                }
            ...
            public override Node AppendInPlace(T item)
            {
                ...
                if (_right.Count < MAXLEAF && !_right.Shared && _right is LeafNode rightLeaf)
                {
                    ...
                    if (c == rightLeaf._items.Length)
                    {
                        ...
                        Array.Copy(rightLeaf._items, 0, newItems, 0, c);
                        rightLeaf._items = newItems;
                    }

                    rightLeaf._items[c] = item;
                    rightLeaf._count += 1;
                    _count += 1;
                    return this;
                }
            ...
            //            public override void Print(string prefixNode, string prefixChildren)
            //            {
            //                ConsoleText.WriteLine("{0}CONCAT {1} {2} count={3} depth={4}", prefixNode, shared ? "S" : " ", IsBalanced() ? "B" : IsAlmostBalanced() ? "A" : " ", _count, _depth);
