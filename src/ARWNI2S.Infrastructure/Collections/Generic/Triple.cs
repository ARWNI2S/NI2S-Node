    ...
    public struct Triple<TFirst, TSecond, TThird> : IComparable, IComparable<Triple<TFirst, TSecond, TThird>>
    {
        ...
        public int CompareTo(Triple<TFirst, TSecond, TThird> other)
        {
            ...
            catch (ArgumentException)
            {
                ...
                {
                    throw new NotSupportedException(string.Format(LocalizedStrings.Collections_UncomparableType, typeof(TFirst).FullName));
                }
                else if (!typeof(IComparable<TSecond>).IsAssignableFrom(typeof(TSecond)) &&
                    !typeof(IComparable).IsAssignableFrom(typeof(TSecond)))
                {
                    throw new NotSupportedException(string.Format(LocalizedStrings.Collections_UncomparableType, typeof(TSecond).FullName));
                }
                else if (!typeof(IComparable<TThird>).IsAssignableFrom(typeof(TThird)) &&
                    !typeof(IComparable).IsAssignableFrom(typeof(TThird)))
                {
                    throw new NotSupportedException(string.Format(LocalizedStrings.Collections_UncomparableType, typeof(TThird).FullName));
                }
                else
