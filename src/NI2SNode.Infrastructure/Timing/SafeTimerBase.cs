...
namespace NI2S.Node.Timing
{
    ...
    internal class SafeTimerBase : IDisposable
    {
        ...
        public void DisposeTimer()
        {
            if (timer != null)
            {
                try
                {
                    ...
                    timer = null;
                    //TODO: Localization.
                    logger?.Verbose(TraceCode.TimerDisposing, "Disposing timer {0}", GetFullName());
                    t.Dispose();
                ...
                {
                    //TODO: Localization.
                    logger?.Warn(TraceCode.TimerDisposeError, string.Format("Ignored error disposing timer {0}", GetFullName()), exc);
                }
            }
        }
        ...
        {
            //TODO: use lastCheckTime or previousTickTime
        ...
        //TODO: REMOVE UNUSED IF REMAIN UNUSED
        private bool Change(TimeSpan newDueTime, TimeSpan period)
        {
            ...
            logger?.Verbose(TraceCode.TimerChanging, LocalizedStrings.TimerChanging, GetFullName(), newDueTime, period);
        ...
        private void HandleSyncTimerCallback(object? state)
        {
            try
            {
                logger?.Verbose3(TraceCode.TimerBeforeCallback, LocalizedStrings.TimerBeforeCallbackSync, GetFullName());
                syncCallbackFunc?.Invoke(state);
                logger?.Verbose3(TraceCode.TimerAfterCallback, LocalizedStrings.TimerAfterCallbackSync, GetFullName());
            }
            catch (Exception exc)
            {
        ...
        private async void HandleAsyncTaskTimerCallback(object? state)
        {
            ...
            try
            {
                logger?.Verbose3(TraceCode.TimerBeforeCallback, LocalizedStrings.TimerBeforeCallbackAsync, GetFullName());
                if (asynTaskCallback != null)
                    await asynTaskCallback(state);
                logger?.Verbose3(TraceCode.TimerAfterCallback, LocalizedStrings.TimerAfterCallbackAsync, GetFullName());
            }
            catch (Exception exc)
            {
        ...
        private void QueueNextTimerTick()
        {
            try
            {
                ...
                logger?.Verbose3(TraceCode.TimerChanging, LocalizedStrings.TimerChanging_Queue, GetFullName());
                ...
                if (timerFrequency == Constants.INFINITE_TIMESPAN)
                {
                    ...
                    logger?.Verbose(TraceCode.TimerStopped, LocalizedStrings.TimerStopped, GetFullName());
                }
                else
                {
                    timer.Change(timerFrequency, Constants.INFINITE_TIMESPAN);

                    logger?.Verbose3(TraceCode.TimerNextTick, LocalizedStrings.TimerNextTick, GetFullName(), timerFrequency);
                }
            }
