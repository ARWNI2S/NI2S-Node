...
namespace ARWNI2S.Infrastructure.Collections
{
    ...
    internal class LRU<TKey, TValue> : IEnumerable<KeyValuePair<TKey, TValue>> where TKey : class
    {
        ...
        internal delegate TValue FetchValueDelegate(TKey key);

        // The following machinery is used to notify node client objects when a key and its value 
        // is being flushed from the cache.
        // The node client's event handler is called after the key has been removed from the cache,
        // but when the cache is in a consistent state so that other methods on the cache may freely
        // be invoked.
        internal class FlushEventArgs : EventArgs
        {
