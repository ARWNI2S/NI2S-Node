...
namespace ARWNI2S.Engine.Environment
{
    ...
    /// </summary>
    public sealed class NI2SFileProvider : PhysicalFileProvider, INiisFileProvider
    {
        #region Accessor
        ...
        public static INiisFileProvider Default
        {
            ...
            internal set { _defaultFileProvider = value; }
        }
        ...
        public NI2SFileProvider(INiisHostEnvironment niisHostEnvironment)
        ...
        {
            RootPath = File.Exists(niisHostEnvironment.NodeRootPath)
                ? Path.GetDirectoryName(niisHostEnvironment.NodeRootPath)
                : niisHostEnvironment.NodeRootPath;
        }
        ...
        public string Combine(params string[] paths)
        {
            ...
            if (System.Environment.OSVersion.Platform == PlatformID.Unix && !IsUncPath(path))
        ...
        public string GetAbsolutePath(params string[] paths)
        {
            ...
            if (paths.Length != 0 && !paths[0].Contains(RootPath, StringComparison.InvariantCulture))
        ...
        public string GetVirtualPath(string path)
        {
            ...
            path = path?.Replace(RootPath, string.Empty).Replace('\\', '/').Trim('/').TrimStart('~', '/');
