...
namespace ARWNI2S.Engine.Plugins
{
    ...
    public partial class PluginDescriptor : PluginDescriptorBaseInfo, IDescriptor, IComparable<PluginDescriptor>
    {
        ...
        public virtual void Save()
        {
            ...
            //in order to avoid possible problems we use NI2SFileProvider.Default
            //instead of the main file provider
            var fileProvider = NI2SFileProvider.Default;

            //get the description file path
            if (OriginalAssemblyFile == null)
                throw new NI2SException($"Cannot load original assembly path for {SystemName} plugin.");
            ...
            if (!fileProvider.FileExists(filePath))
                throw new NI2SException($"Description file for {SystemName} plugin does not exist. {filePath}");
