...
namespace ARWNI2S.Engine.Plugins
{
    ...
    /// </summary>
    internal static class PluginsInitializer
    {
        #region Fields

        private static readonly INiisFileProvider _fileProvider;
        ...
        static PluginsInitializer()
        {
            ...
            if (!AppDomain.CurrentDomain.BaseDirectory.Equals(System.Environment.CurrentDirectory, StringComparison.InvariantCultureIgnoreCase))
                foreach (var file in _fileProvider.GetFiles(System.Environment.CurrentDirectory, "*.dll"))
                    _baseAppLibraries.Add(new KeyValuePair<string, Version>(_fileProvider.GetFileName(file), GetAssemblyVersion(file)));

            //get all libraries from refs directory
            var refsPathName = _fileProvider.Combine(System.Environment.CurrentDirectory, PluginDefaults.RefsPathName);
        ...
        private static bool IsAlreadyLoaded(string filePath, string pluginName)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(fileNameWithoutExtension))
                    throw new NI2SException($"Cannot get file extension for {fileName}");
            ...
            using (new ReaderWriteLockDisposable(_locker))
            {
                ...
                catch (Exception exception)
                {
                    ...
                    for (var inner = exception; inner != null; inner = inner.InnerException)
                        message = $"{message}{inner.Message}{System.Environment.NewLine}";

                    throw new NI2SException(message, exception);
                }
