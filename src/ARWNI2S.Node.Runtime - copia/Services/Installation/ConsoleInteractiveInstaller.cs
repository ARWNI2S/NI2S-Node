...
{
    internal class ConsoleInteractiveInstaller : IClusterInstaller
    {
        ...
        public ConsoleInteractiveInstaller(NI2SSettings nI2SSettings,
        ...
        public async Task InstallDatabaseAsync()
        {
            ...
            try
            {
                ...
                {
                    (string admin, string password) = await PromptForUserAndPasswordAsync();
                    await _installationService.Value.InstallNodeAsync(admin, password);
                }
