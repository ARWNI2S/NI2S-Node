...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        private void ScanAssemblyAndRegisterStartup(NodeHostBuilderContext context, IServiceCollection services, NodeHostBuilderContext nodehostContext, NodeHostOptions nodeHostOptions)
        {
            try
            {
        ...
        public INodeHostBuilder UseStartup<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicMethods)] TStartup>(Func<NodeHostBuilderContext, TStartup> startupFactory)
        {
            ...
            void ConfigureStartup(NodeHostBuilderContext context, IServiceCollection services)
            {
                // UseStartup can be called multiple times. Only run the last one.
                if (ReferenceEquals(_startupObject, startupFactory))
                {
                    var nodeNodeHostBuilderContext = GetNodeHostBuilderContext(context);
                    var instance = startupFactory(nodeNodeHostBuilderContext) ?? throw new InvalidOperationException("The specified factory returned null startup instance.");
        ...
        private void UseStartup([DynamicallyAccessedMembers(StartupLinkerOptions.Accessibility)] Type startupType, NodeHostBuilderContext context, IServiceCollection services, object instance = null)
        {
            var nodeNodeHostBuilderContext = GetNodeHostBuilderContext(context);
            var nodeHostOptions = (NodeHostOptions)context.Properties[typeof(NodeHostOptions)];
            ...
            try
            {
                ...
                instance ??= ActivatorUtilities.CreateInstance(new HostServiceProvider(nodeNodeHostBuilderContext), startupType);
            ...
            static void InvokeContainer(GenericNodeHostBuilder genericNodeHostBuilder, ConfigureContainerBuilder configureContainerBuilder)
            {
                ...
                var actionType = typeof(Action<,>).MakeGenericType(typeof(NodeHostBuilderContext), containerType);
        ...
        private void ConfigureContainerImpl<TContainer>(NodeHostBuilderContext context, TContainer container) where TContainer : notnull
        {
