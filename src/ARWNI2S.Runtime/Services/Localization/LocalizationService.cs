...
using ARWNI2S.Data.Entities.Security;
using ARWNI2S.Data.Extensions;
using ARWNI2S.Entities.Globalization;
using ARWNI2S.ExportImport;
using ARWNI2S.Node;
using ARWNI2S.Plugins;
using ARWNI2S.Services.Configuration;
...
#pragma warning restore CA1862 // Use las sobrecargas del método "StringComparison" para realizar comparaciones de cadenas sin distinción de mayúsculas y minúsculas
        ...
        {
            var key = _staticCacheManager.PrepareKeyForDefaultCache(LocalizationServicesDefaults.LocaleStringResourcesAllCacheKey, languageId);
            ...
            if (!loadPublicLocales.HasValue || allLocales != null)
            {
                ...
                await _staticCacheManager.RemoveAsync(LocalizationServicesDefaults.LocaleStringResourcesAllPublicCacheKey, languageId);
                await _staticCacheManager.RemoveAsync(LocalizationServicesDefaults.LocaleStringResourcesAllAdminCacheKey, languageId);
            ...
            key = _staticCacheManager.PrepareKeyForDefaultCache(loadPublicLocales.Value
                ? LocalizationServicesDefaults.LocaleStringResourcesAllPublicCacheKey
                : LocalizationServicesDefaults.LocaleStringResourcesAllAdminCacheKey,
                languageId);
            ...
            return await _staticCacheManager.GetAsync(key, () =>
            {
                ...
                var query = from l in _lsrRepository.Table
                            ...
                            where l.LanguageId == languageId
                            select l;
                query = loadPublicLocales.Value ? query.Where(r => !r.ResourceName.StartsWith(LocalizationServicesDefaults.AdminLocaleStringResourcesPrefix)) : query.Where(r => r.ResourceName.StartsWith(LocalizationServicesDefaults.AdminLocaleStringResourcesPrefix));
            ...
            bool logIfNotFound = true, string defaultValue = "", bool returnEmptyIfNotFound = false)
        {
            ...
            if (_localizationSettings.LoadAllLocaleRecordsOnStartup)
            {
                ...
                var resources = await GetAllResourceValuesAsync(languageId, !resourceKey.StartsWith(LocalizationServicesDefaults.AdminLocaleStringResourcesPrefix, StringComparison.InvariantCultureIgnoreCase));
            ...
            {
                //gradual loading
                var key = _staticCacheManager.PrepareKeyForDefaultCache(LocalizationServicesDefaults.LocaleStringResourcesByNameCacheKey, languageId, resourceKey);
        ...
        public virtual async Task ImportResourcesFromXmlAsync(Language language, StreamReader xmlStreamReader, bool updateExistingResources = true)
        {
            ...
            {
                if (lsNamesList.TryGetValue(name, out var val))
                {
        ...
        ///// <summary>
        ///// Get localized property of game setting
        ///// </summary>
        ///// <typeparam name="TSettings">Settings type</typeparam>
        ///// <param name="settings">Settings</param>
        ///// <param name="keySelector">Key selector</param>
        ...
        ///// The task result contains the localized property
        ///// </returns>
        //public async Task<string> GetLocalizedGameSettingAsync<TSettings>(TSettings settings, Expression<Func<TSettings, string>> keySelector,
        //    int languageId, bool returnDefaultValue = true, bool ensureTwoPublishedLanguages = true)
        //    where TSettings : IGameplaySettings, new()
        //{
        ...
        ///// <summary>
        ///// Save localized property of game setting
        ///// </summary>
        ///// <typeparam name="TSettings">Settings type</typeparam>
        ///// <param name="settings">Settings</param>
        ///// <param name="keySelector">Key selector</param>
        ...
        ///// The task result contains the localized property
        ///// </returns>
        //public async Task SaveLocalizedGameSettingAsync<TSettings>(TSettings settings, Expression<Func<TSettings, string>> keySelector,
        ...
        public virtual async Task<string> GetLocalizedEnumAsync<TEnum>(TEnum enumValue, int? languageId = null) where TEnum : struct
        {
            ...
            //localized value
            var workingLanguage = await _languageService.GetLanguageByCultureAsync(await _workContext.GetWorkingCultureAsync());
            var resourceName = $"{LocalizationServicesDefaults.EnumLocaleStringResourcesPrefix}{typeof(TEnum)}.{enumValue}";
