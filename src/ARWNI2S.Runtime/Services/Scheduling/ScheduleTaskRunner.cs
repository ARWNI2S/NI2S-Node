        ...
        protected async Task PerformTaskAsync(ScheduleTask scheduleTask)
        {
            var type = (Type.GetType(scheduleTask.Type) ??
                       ...
                       AppDomain.CurrentDomain.GetAssemblies()
                           ...
                           .FirstOrDefault(t => t != null)) ?? throw new NiisException($"Schedule task ({scheduleTask.Type}) cannot by instantiated");
            object instance = null;

            try
            {
                instance = NI2SEngineContext.Current.Resolve(type);
            }
            catch
            {
                // ignored
            ...
            instance ??= NI2SEngineContext.Current.ResolveUnregistered(type);
