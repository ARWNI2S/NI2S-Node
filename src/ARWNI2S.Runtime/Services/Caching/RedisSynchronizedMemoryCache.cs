...
namespace ARWNI2S.Services.Caching
{
    ...
    public class RedisSynchronizedMemoryCache : ISynchronizedMemoryCache
    {
        ...
        {
            var channel = new RedisChannel(ChannelPrefix + "*", RedisChannel.PatternMode.Pattern);
        ...
        protected void PublishChangeEvent(object key)
        {
            ...
            var channel = new RedisChannel($"{ChannelPrefix}{stringKey}", RedisChannel.PatternMode.Literal);

            subscriber.Publish(channel, _processId, CommandFlags.FireAndForget);
        }
