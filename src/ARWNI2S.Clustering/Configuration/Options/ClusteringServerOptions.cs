...
{
    /// <summary>
    /// Provides programmatic configuration of Clustering-specific features.
    /// </summary>
    public class ClusteringServerOptions
    {
        internal const string DisableHttp1LineFeedTerminatorsSwitchKey = "Microsoft.AspNetCore.Server.Clustering.DisableHttp1LineFeedTerminators";
        private const string FinOnErrorSwitch = "Microsoft.AspNetCore.Server.Clustering.FinOnError";
        internal const string CertificateFileWatchingSwitch = "Microsoft.AspNetCore.Server.Clustering.DisableCertificateFileWatching";
        private static readonly bool _finOnError;
        private static readonly bool _disableCertificateFileWatching;

        static ClusteringServerOptions()
        {
        ...
        //// The following two lists configure the endpoints that Clustering should listen to. If both lists are empty, the "urls" config setting (e.g. UseUrls) is used.
        ...
        ///// <summary>
        ///// Gets or sets a value that controls whether synchronous IO is allowed for the <see cref="NiisContext.Request"/> and <see cref="NiisContext.Response"/>
        ///// </summary>
        ///// <remarks>
        ///// Defaults to false.
        ///// </remarks>
        ...
        ///// Defaults to false.
        ///// </remarks>
        //[Obsolete($"This property is obsolete and will be removed in a future version. It no longer has any impact on runtime behavior. Use {nameof(Microsoft.AspNetCore.Server.Clustering.Core.ListenOptions)}.{nameof(Microsoft.AspNetCore.Server.Clustering.Core.ListenOptions.DisableAltSvcHeader)} to configure \"Alt-Svc\" behavior.", error: true)]
        ...
        /// Typically initialized by UseClustering().
        /// </summary>
        ...
        ///// </summary>
        //public ClusteringServerLimits Limits { get; } = new ClusteringServerLimits();

        /// <summary>
        /// Provides a configuration source where endpoints will be loaded from on server start.
        /// The default is <see langword="null"/>.
        /// </summary>
        public ClusteringConfigurationLoader ConfigurationLoader { get; set; }
        ...
        internal bool EnableWebTransportAndH3Datagrams
        {
            get
            {
                ...
                {
                    _enableWebTransportAndH3Datagrams = AppContext.TryGetSwitch("Microsoft.AspNetCore.Server.Clustering.Experimental.WebTransportAndH3Datagrams", out var enabled) && enabled;
                }
        ...
        //{
        //    listenOptions.ClusteringServerOptions = this;
        ...
        //        var logger = ApplicationServices!.GetRequiredService<ILogger<ClusteringServer>>();
        ...
        //private static X509Certificate2 GetDevelopmentCertificateFromStore(ILogger<ClusteringServer> logger)
        //{
        //    try
        //    {
        ...
        //        // On MacOS, this may cause a UI prompt, since it requires accessing the keychain.  Clustering must NEVER
        ...
        /// <summary>
        /// Creates a configuration loader for setting up Clustering.
        /// </summary>
        /// <returns>A <see cref="ClusteringConfigurationLoader"/> for configuring endpoints.</returns>
        public ClusteringConfigurationLoader Configure() => Configure(new ConfigurationBuilder().Build());

        /// <summary>
        /// Creates a configuration loader for setting up Clustering that takes an <see cref="IConfiguration"/> as input.
        /// This configuration must be scoped to the configuration section for Clustering.
        /// Call <see cref="Configure(IConfiguration, bool)"/> to enable dynamic endpoint binding updates.
        /// </summary>
        /// <param name="config">The configuration section for Clustering.</param>
        /// <returns>A <see cref="ClusteringConfigurationLoader"/> for further endpoint configuration.</returns>
        public ClusteringConfigurationLoader Configure(IConfiguration config) => Configure(config, reloadOnChange: false);
        ...
        /// If <see langword="true"/>, Clustering will dynamically update endpoint bindings when configuration changes.
        ...
        /// </param>
        /// <returns>A <see cref="ClusteringConfigurationLoader"/> for further endpoint configuration.</returns>
        public ClusteringConfigurationLoader Configure(IConfiguration config, bool reloadOnChange)
        {
            if (ApplicationServices is null)
            {
                throw new InvalidOperationException($"{nameof(ApplicationServices)} must not be null. This is normally set automatically via {nameof(IConfigureOptions<ClusteringServerOptions>)}.");
            }
            ...
            //    : null;
            var loader = new ClusteringConfigurationLoader(this, config, /*httpsConfigurationService, certificatePathWatcher,*/ reloadOnChange);
