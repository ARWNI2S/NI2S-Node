...
namespace NI2S.Node.Command
{
        ...
        where TKey : notnull
    {
        ...
        public CommandMiddleware(IServiceProvider serviceProvider, IOptions<CommandOptions> commandOptions, IPackageMapper<TNetPackageInfo, TPackageInfo>? packageMapper = null)
        {
            ...
            var availableSessionTypes = new List<Type> { typeof(ISession), sessionType };
            ...
            commandSetFactories.AddRange(commandOptions.Value.GetCommandTypes(t => true).Select((t) =>
            {
                ...
                {
                    CommandTypeInfo face = commandInterfaces[i];

                    if (face.CommandType != null && face.CommandType.IsAssignableFrom(t))
        ...
        private void RegisterCommandSetFactoriesFromServices(List<ICommandSetFactory?> commandSetFactories, IServiceProvider serviceProvider, Type commandType, Type commandSetFactoryType, Func<Type, Type> commandWrapFactory)
        {
            foreach (var command in serviceProvider.GetServices(commandType).OfType<ICommand>())
            {
                ...
                if (commandWrapFactory != null)
                {
                    ...
                    cmd = ActivatorUtilities.CreateInstance(serviceProvider, commandWrapType, command) as ICommand;
                }

                var commandTypeInfo = new CommandTypeInfo(cmd);
                commandTypeInfo.ActualCommandType = actualCommandType;
                commandSetFactories.Add(ActivatorUtilities.CreateInstance(serviceProvider, commandSetFactoryType, commandTypeInfo) as ICommandSetFactory);
            }
        }
        ...
        protected virtual async ValueTask HandlePackage(ISession session, TPackageInfo package)
        {
        ...
        protected virtual async Task OnPackageReceived(ISession session, TPackageInfo package)
        {
            await HandlePackage(session, package);
        }

        ValueTask IPackageHandler<TNetPackageInfo>.Handle(ISession session, TNetPackageInfo package)
        {
        ...
        interface ICommandSet
        {
            ...
            ValueTask ExecuteAsync(ISession session, TPackageInfo package);
        }
        ...
        class CommandTypeInfo
        {
            ...
            public CommandTypeInfo(Type? commandType, Type? commandSetFactoryType, bool wrapRequired = false)
            {
            ...
            {
                var commandTyeInfo = new CommandTypeInfo(WrapRequired && WrapFactory != null ? WrapFactory(type) : type, null)
                {
                    ActualCommandType = type
                };
                return (ICommandSetFactory)ActivatorUtilities.CreateInstance(null!, CommandSetFactoryType!, commandTyeInfo);
            }
        }
            ...
            where TAppSession : ISession
        {
            ...
            public async ValueTask ExecuteAsync(ISession session, TPackageInfo package)
            {
            ...
            private async ValueTask ExecuteAsyncWithFilter(ISession session, TPackageInfo package)
            {
                ...
                var command = AsyncCommand ?? Command as ICommand;
