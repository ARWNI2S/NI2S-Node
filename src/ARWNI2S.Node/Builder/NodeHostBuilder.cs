...
using NodeHost = ARWNI2S.Node.Hosting.Internal.NodeHost;
    ...
    public class NodeHostBuilder : INodeHostBuilder
    {
        ...
        public NodeHostBuilder()
        {
            ...
            _config = new ConfigurationBuilder()
                .AddEnvironmentVariables(prefix: "ARWNI2S_")
                .Build();
            ...
            if (string.IsNullOrEmpty(GetSetting(NodeHostDefaults.EnvironmentKey)))
            {
                ...
                UseSetting(NodeHostDefaults.EnvironmentKey, Environment.GetEnvironmentVariable("Hosting:Environment")
                    ?? Environment.GetEnvironmentVariable("NI2S_ENV"));
            }

            if (string.IsNullOrEmpty(GetSetting(NodeHostDefaults.ServerUrlsKey)))
            {
                // Try adding legacy url key, never remove this.
                UseSetting(NodeHostDefaults.ServerUrlsKey, Environment.GetEnvironmentVariable("ARWNI2S_SERVER.URLS"));
            }
        ...
        /// <summary>
        /// Adds a delegate for configuring additional services for the host or node application. This may be called
        /// multiple times.
        /// </summary>
        ...
        /// <summary>
        /// Builds the required services and an <see cref="INodeHost"/> which hosts a node application.
        /// </summary>
        public INodeHost Build()
        {
            if (_nodeHostBuilt)
            {
                throw new InvalidOperationException(ErrorStrings.NodeHostBuilder_SingleInstance);
            }
            _nodeHostBuilt = true;
            ...
            if (!_options.SuppressStatusMessages)
            {
                ...
                {
                    Console.WriteLine("The environment variable 'Hosting:Environment' is obsolete and has been replaced with 'ARWNI2S_ENVIRONMENT'");
                }

                if (Environment.GetEnvironmentVariable("NI2S_ENV") != null)
                {
                    Console.WriteLine("The environment variable 'NI2S_ENV' is obsolete and has been replaced with 'ARWNI2S_ENVIRONMENT'");
                }

                if (Environment.GetEnvironmentVariable("ARWNI2S_SERVER.URLS") != null)
                {
                    Console.WriteLine("The environment variable 'ARWNI2S_SERVER.URLS' is obsolete and has been replaced with 'ARWNI2S_URLS'");
                }
            }
...
#pragma warning disable CS0618 // Type or member is obsolete
            services.AddSingleton<Hosting.IHostingEnvironment>(_hostingEnvironment);
            services.AddSingleton<Microsoft.Extensions.Hosting.IHostingEnvironment>(_hostingEnvironment);
#pragma warning restore CS0618 // Type or member is obsolete
            ...
            services.AddTransient<INiisContextFactory, DefaultNiisContextFactory>();
        ...
        private void ScanAssemblyAndRegisterStartup(ServiceCollection services, string startupAssemblyName)
        {
            try
            {
                ...
                {
                    services.AddSingleton(typeof(INodeStartup), startupType);
                }
                else
                {
                    services.AddSingleton(typeof(INodeStartup), RegisterStartup);
            ...
            catch (Exception ex)
            {
                ...
                services.AddSingleton<INodeStartup>(_ =>
                {
                    capture.Throw();
                    return null;
                });
