...
namespace ARWNI2S.Node.Hosting.Extensions
{
    internal sealed partial class ManifestLocalAssetFileProvider : IFileProvider
    {
        ...
        private readonly LocalAssetNode _root;

        public ManifestLocalAssetFileProvider(NodeAssetManifest manifest, Func<string, IFileProvider> fileProviderFactory)
        {
        ...
        public IDirectoryContents GetDirectoryContents(string subpath)
        {
            ...
            else
            {
                ...
                return new NodeAssetsDirectoryContents((files as IEnumerable<IFileInfo>) ?? Array.Empty<IFileInfo>());
            }
            ...
            void GetCandidateFilesForNode(LocalAssetNode candidate, HashSet<IFileInfo> files)
            {
                foreach (var child in candidate.Children!)
                {
                    ...
                    {
                        // This is a folder
                        var file = new NodeAssetsDirectoryInfo(child.Key);
                    ...
                    else
                    {
                        ...
                        files.Add(string.Equals(child.Key, match.Path, _fsComparison) ? file :
                            ...
                            new NodeAssetsFileInfo(child.Key, file));
                    }
                }
            }
        }
        ...
        public IFileInfo GetFileInfo(string subpath)
        {
            ...
            LocalAssetNode candidate = _root;
            List<NodeAssetPattern> patterns = null;
            ...
            if (match != null)
            {
                ...
                {
                    return new NodeAssetsFileInfo(segments[^1], file);
                }
            }
        ...
        private sealed class NodeAssetsDirectoryContents : IDirectoryContents
        {
            private readonly IEnumerable<IFileInfo> _files;

            public NodeAssetsDirectoryContents(IEnumerable<IFileInfo> files) =>
                _files = files;
        ...
        private sealed class NodeAssetsDirectoryInfo : IFileInfo
        {
            private static readonly DateTimeOffset _lastModified = DateTimeOffset.FromUnixTimeSeconds(0);

            public NodeAssetsDirectoryInfo(string name)
            {
                Name = name;
            }
        ...
        private sealed class NodeAssetsFileInfo : IFileInfo
        {
            private readonly IFileInfo _source;

            public NodeAssetsFileInfo(string name, IFileInfo source)
            {
                Name = name;
                _source = source;
            }
        ...
        internal sealed class NodeAssetManifest
        {
            ...
            internal static NodeAssetManifest Parse(Stream manifest)
            {
                return JsonSerializer.Deserialize(
                    manifest,
                    SourceGenerationContext.DefaultWithConverter.NodeAssetManifest)!;
            }
        }

        [JsonSourceGenerationOptions]
        [JsonSerializable(typeof(NodeAssetManifest))]
        ...
        internal sealed class LocalAssetNode
        {
            [JsonPropertyName("Asset")]
            public NodeAssetMatch Match { get; set; }

            [JsonConverter(typeof(OSBasedCaseConverter))]
            public Dictionary<string, LocalAssetNode> Children { get; set; }

            public NodeAssetPattern[] Patterns { get; set; }
        ...
        internal sealed class NodeAssetMatch
        {
        ...
        internal sealed class NodeAssetPattern
        {
            [JsonPropertyName("ContentRootIndex")]
            public int ContentRoot { get; set; }
        ...
        private sealed class OSBasedCaseConverter : JsonConverter<Dictionary<string, LocalAssetNode>>
        {
            public override Dictionary<string, LocalAssetNode> Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
            {
                // Need to recursively deserialize `Dictionary<string, LocalAssetNode>` but can't deserialize
                ...
                var parsed = JsonSerializer.Deserialize(ref reader, SourceGenerationContext.DefaultWithConverter.IDictionaryStringLocalAssetNode)!;
                var result = new Dictionary<string, LocalAssetNode>(NodeAssetManifest.PathComparer);
                MergeChildren(parsed, result);
                return result;

                static void MergeChildren(
                    IDictionary<string, LocalAssetNode> newChildren,
                    IDictionary<string, LocalAssetNode> existing)
                {
                        ...
                        else
                        {
                            if (value.Patterns != null)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        var newList = new NodeAssetPattern[existingNode.Patterns.Length + value.Patterns.Length];
            ...
            public override void Write(Utf8JsonWriter writer, Dictionary<string, LocalAssetNode> value, JsonSerializerOptions options)
            {
