...
{
    /// <summary>
    /// A non-buildable <see cref="INiisHostBuilder"/> for <see cref="NI2SNodeBuilder"/>.
    ...
    public sealed class ConfigureNI2SHostBuilder : INiisHostBuilder
    {
        ...
        public INiisHostBuilder ConfigureNI2SConfiguration(Action<NI2SHostBuilderContext, IConfigurationBuilder> configureDelegate)
        {
                ...
                && !string.Equals(HostingPathResolver.ResolvePath(previousNiisRoot, previousContentRoot), HostingPathResolver.ResolvePath(_configuration[NI2SHostingDefaults.NI2SRootKey], previousContentRoot), StringComparison.OrdinalIgnoreCase))
            {
                ...
                throw new NotSupportedException($"The web root changed from \"{HostingPathResolver.ResolvePath(previousNiisRoot, previousContentRoot)}\" to \"{HostingPathResolver.ResolvePath(_configuration[NI2SHostingDefaults.NI2SRootKey], previousContentRoot)}\". Changing the host configuration using NI2SHostBuilder.NI2SHost is not supported. Use NI2SHost.CreateBuilder(NI2SNodeOptions) instead.");
            }
            ...
            {
                // Disallow changing any host configuration
                throw new NotSupportedException($"The hosting startup assemblies exclude list changed from \"{previousHostingStartupAssembliesExclude}\" to \"{_configuration[NI2SHostingDefaults.HostingStartupExcludeAssembliesKey]}\". Changing the host configuration using NI2SHostBuilder.NI2SHost is not supported. Use NI2SHost.CreateBuilder(NI2SNodeOptions) instead.");
            }
