...
namespace ARWNI2S.Node.Hosting.Internal
{
    ...
    /// </summary>
    public sealed class ConfigureNodeHostBuilder : INodeHostBuilder, ISupportsNodeStartup
    {
        ...
        public INodeHostBuilder ConfigureAppConfiguration(Action<NodeHostBuilderContext, IConfigurationBuilder> configureDelegate)
        {
            ...
            else if (!string.Equals(previousEngine, _configuration[NodeHostDefaults.ApplicationKey], StringComparison.OrdinalIgnoreCase))
            {
                ...
                throw new NotSupportedException($"The engine name changed from \"{previousEngine}\" to \"{_configuration[NodeHostDefaults.ApplicationKey]}\". Changing the host configuration using NodeEngineBuilder.NodeHost is not supported. Use NodeEngine.CreateBuilder(NodeEngineOptions) instead.");
            }
        ...
        INodeHostBuilder ISupportsNodeStartup.Configure(Action<IEngineBuilder> configure)
        {
        ...
        INodeHostBuilder ISupportsNodeStartup.Configure(Action<NodeHostBuilderContext, IEngineBuilder> configure)
        {
            throw new NotSupportedException("Configure() is not supported by NodeEngineBuilder.NodeHost. Use the NodeEngine returned by NodeEngineBuilder.Build() instead.");
        }

        INodeHostBuilder ISupportsNodeStartup.UseStartup([DynamicallyAccessedMembers(StartupLinkerOptions.Accessibility)] Type startupType)
        {
        ...
        INodeHostBuilder ISupportsNodeStartup.UseStartup<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicMethods)] TStartup>(Func<NodeHostBuilderContext, TStartup> startupFactory)
        {
            throw new NotSupportedException("UseStartup() is not supported by NodeEngineBuilder.NodeHost. Use the NodeEngine returned by NodeEngineBuilder.Build() instead.");
        }
    }
