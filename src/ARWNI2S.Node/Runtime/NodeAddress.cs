...
using ARWNI2S.Engine.Serialization;
using System;
...
{
    /// <summary>
    /// Data class encapsulating the details of node addresses.
    /// </summary>
    [Serializable, Immutable]
    [JsonConverter(typeof(NodeAddressConverter))]
    [DebuggerDisplay("NodeAddress {ToString()}")]
    [SuppressReferenceTracking]
    public sealed class NodeAddress : IEquatable<NodeAddress>, IComparable<NodeAddress>, ISpanFormattable
    {
        [NonSerialized]
        ...
        private static readonly Weak<(IPAddress Address, int Port, int Generation), NodeAddress> nodeAddressInterningCache = new(InternerConstants.SIZE_MEDIUM);

        /// <summary>Gets the special constant value which indicate an empty <see cref="NodeAddress"/>.</summary>
        public static NodeAddress Zero { get; } = New(new IPAddress(0), 0, 0);

        /// <summary>
        /// Factory for creating new NodeAddresses with specified IP endpoint address and node generation number.
        /// </summary>
        /// <param name="ep">IP endpoint address of the node.</param>
        /// <param name="gen">Generation number of the node.</param>
        /// <returns>NodeAddress object initialized with specified address and node generation.</returns>
        public static NodeAddress New(IPEndPoint ep, int gen)
        {
            return nodeAddressInterningCache.FindOrCreate((ep.Address, ep.Port, gen),
                // Normalize endpoints
        ...
        /// <param name="generation">Generation number of the node.</param>
        /// <returns>NodeAddress object initialized with specified address and node generation.</returns>
        public static NodeAddress New(IPAddress address, int port, int generation)
        {
            return nodeAddressInterningCache.FindOrCreate((address, port, generation),
                // Normalize endpoints
        ...
        private NodeAddress(IPEndPoint endpoint, int generation)
        {
            Endpoint = endpoint;
        ...
        /// <summary> Allocate a new node generation number. </summary>
        /// <returns>A new node generation number.</returns>
        ...
        /// <summary>
        /// Return this NodeAddress in a standard string form, suitable for later use with the <c>FromParsableString</c> method.
        /// </summary>
        /// <returns>NodeAddress in a standard string format.</returns>
        public string ToParsableString()
        {
            // This must be the "inverse" of FromParsableString, and must be the same across all nodes in a deployment.
            // Basically, this should never change unless the data content of NodeAddress changes
        ...
        /// <summary>
        /// Create a new NodeAddress object by parsing string in a standard form returned from <c>ToParsableString</c> method.
        /// </summary>
        /// <param name="addr">String containing the NodeAddress info to be parsed.</param>
        /// <returns>New NodeAddress object created from the input data.</returns>
        public static NodeAddress FromParsableString(string addr)
        {
            // This must be the "inverse" of ToParsableString, and must be the same across all nodes in a deployment.
            // Basically, this should never change unless the data content of NodeAddress changes
            ...
            if (atSign < 0 || lastColon < 0) throw new FormatException("Invalid string NodeAddress: " + addr);
        ...
        /// <param name="addr">String containing the NodeAddress info to be parsed.</param>
        /// <returns>New NodeAddress object created from the input data.</returns>
        public static NodeAddress FromUtf8String(ReadOnlySpan<byte> addr)
        {
            // This must be the "inverse" of ToParsableString, and must be the same across all nodes in a deployment.
            // Basically, this should never change unless the data content of NodeAddress changes
            ...
            if (atSign < 0) ThrowInvalidUtf8NodeAddress(addr);

            // IPEndpoint is the host, then ':', then the port
            var endpointSlice = addr[..atSign];
            int lastColon = endpointSlice.LastIndexOf((byte)':');
            if (lastColon < 0) ThrowInvalidUtf8NodeAddress(addr);
            ...
            if (!IPAddress.TryParse(hostString, out var host))
                ThrowInvalidUtf8NodeAddress(addr);
        ...
        [DoesNotReturn]
        private static void ThrowInvalidUtf8NodeAddress(ReadOnlySpan<byte> addr)
            => throw new FormatException("Invalid string NodeAddress: " + Encoding.UTF8.GetString(addr));

        /// <summary>
        /// Return a long string representation of this NodeAddress.
        /// </summary>
        /// <remarks>
        ...
        /// </remarks>
        /// <returns>String representation of this NodeAddress.</returns>
        ...
        /// <summary>
        /// Return a long string representation of this NodeAddress, including it's consistent hash value.
        /// </summary>
        /// <remarks>
        ...
        /// <inheritdoc />
        public override bool Equals(object? obj) => Equals(obj as NodeAddress);

        /// <inheritdoc />
        public override int GetHashCode() => Endpoint.GetHashCode() ^ Generation;

        /// <summary>Returns a consistent hash value for this node address.</summary>
        /// <returns>Consistent hash value for this node address.</returns>
...
#pragma warning restore CS0618
        ...
        /// <summary>
        /// Two node addresses match if they are equal or if one generation or the other is 0.
        /// </summary>
        /// <param name="other"> The other NodeAddress to compare this one with. </param>
        /// <returns>Returns <c>true</c> if the two NodeAddresses are considered to match -- if they are equal or if one generation or the other is 0. </returns>
        internal bool Matches([NotNullWhen(true)] NodeAddress? other)
        {
        ...
        /// <inheritdoc/>
        public bool Equals([NotNullWhen(true)] NodeAddress? other)
        ...
        /// </returns>
        internal bool IsSameLogicalNode([NotNullWhen(true)] NodeAddress? other)
        ...
        /// </returns>
        public bool IsSuccessorOf(NodeAddress other) => IsSameLogicalNode(other) && other.Generation > 0 && Generation > other.Generation;
        ...
        /// </returns>
        public bool IsPredecessorOf(NodeAddress other) => IsSameLogicalNode(other) && Generation > 0 && Generation < other.Generation;
        ...
        public int CompareTo(NodeAddress? other)
        {
            ...
            // and is also semantically meaningful - older nodes (with smaller Generation) will appear first in the comparison order.
...
#pragma warning restore CS0618
    ...
    /// <summary>
    /// Functionality for converting <see cref="NodeAddress"/> instances to and from their JSON representation.
    /// </summary>
    public sealed class NodeAddressConverter : JsonConverter<NodeAddress>
    {
        /// <inheritdoc />
        public override NodeAddress Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options) => NodeAddress.FromParsableString(reader.GetString()!);

        /// <inheritdoc />
        public override void Write(Utf8JsonWriter writer, NodeAddress value, JsonSerializerOptions options) => writer.WriteStringValue(value.ToUtf8String());
    }
}
