...
namespace ARWNI2S.Runtime
{
    internal sealed partial class ManifestStaticNodeAssetFileProvider : IFileProvider
    {
        ...
        private readonly StaticNodeAssetNode _root;

        public ManifestStaticNodeAssetFileProvider(StaticNodeAssetManifest manifest, Func<string, IFileProvider> fileProviderFactory)
        {
        ...
        public IDirectoryContents GetDirectoryContents(string subpath)
        {
            ...
            else
            {
                ...
                return new StaticNodeAssetsDirectoryContents((files as IEnumerable<IFileInfo>) ?? Array.Empty<IFileInfo>());
            }

            HashSet<IFileInfo> GetFilesForCandidatePatterns(string[] segments, StaticNodeAssetNode candidate, HashSet<IFileInfo> files)
            {
            ...
            void GetCandidateFilesForNode(StaticNodeAssetNode candidate, HashSet<IFileInfo> files)
            {
                foreach (var child in candidate.Children!)
                {
                    ...
                    {
                        // This is a folder
                        var file = new StaticNodeAssetsDirectoryInfo(child.Key);
                    ...
                    else
                    {
                        ...
                        files.Add(string.Equals(child.Key, match.Path, _fsComparison) ? file :
                            ...
                            new StaticNodeAssetsFileInfo(child.Key, file));
                    }
                }
            }
        }
        ...
        public IFileInfo GetFileInfo(string subpath)
        {
            ...
            StaticNodeAssetNode candidate = _root;
            List<StaticNodeAssetPattern> patterns = null;
            ...
            if (match != null)
            {
                ...
                // from previous Static Node Assets versions.
                var file = _fileProviders[match.ContentRoot].GetFileInfo(match.Path);
                ...
                {
                    return new StaticNodeAssetsFileInfo(segments[^1], file);
                }
            }
        ...
        private sealed class StaticNodeAssetsDirectoryContents : IDirectoryContents
        {
            private readonly IEnumerable<IFileInfo> _files;

            public StaticNodeAssetsDirectoryContents(IEnumerable<IFileInfo> files) =>
                _files = files;
        ...
        private sealed class StaticNodeAssetsDirectoryInfo : IFileInfo
        {
            private static readonly DateTimeOffset _lastModified = DateTimeOffset.FromUnixTimeSeconds(0);

            public StaticNodeAssetsDirectoryInfo(string name)
            {
                Name = name;
            }
        ...
        private sealed class StaticNodeAssetsFileInfo : IFileInfo
        {
            private readonly IFileInfo _source;

            public StaticNodeAssetsFileInfo(string name, IFileInfo source)
            {
                Name = name;
                _source = source;
            }
        ...
        internal sealed class StaticNodeAssetManifest
        {
            ...
            public StaticNodeAssetNode Root { get; set; } = null!;

            internal static StaticNodeAssetManifest Parse(Stream manifest)
            {
                return JsonSerializer.Deserialize(
                    manifest,
                    SourceGenerationContext.DefaultWithConverter.StaticNodeAssetManifest)!;
            }
        }

        [JsonSourceGenerationOptions]
        [JsonSerializable(typeof(StaticNodeAssetManifest))]
        [JsonSerializable(typeof(IDictionary<string, StaticNodeAssetNode>))]
        ...
        internal sealed class StaticNodeAssetNode
        {
            [JsonPropertyName("Asset")]
            public StaticNodeAssetMatch Match { get; set; }

            [JsonConverter(typeof(OSBasedCaseConverter))]
            public Dictionary<string, StaticNodeAssetNode> Children { get; set; }

            public StaticNodeAssetPattern[] Patterns { get; set; }
        ...
        internal sealed class StaticNodeAssetMatch
        {
        ...
        internal sealed class StaticNodeAssetPattern
        {
            [JsonPropertyName("ContentRootIndex")]
            public int ContentRoot { get; set; }
        ...
        private sealed class OSBasedCaseConverter : JsonConverter<Dictionary<string, StaticNodeAssetNode>>
        {
            public override Dictionary<string, StaticNodeAssetNode> Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
            {
                // Need to recursively deserialize `Dictionary<string, StaticNodeAssetNode>` but can't deserialize
                ...
                var parsed = JsonSerializer.Deserialize(ref reader, SourceGenerationContext.DefaultWithConverter.IDictionaryStringStaticNodeAssetNode)!;
                var result = new Dictionary<string, StaticNodeAssetNode>(StaticNodeAssetManifest.PathComparer);
                MergeChildren(parsed, result);
                return result;

                static void MergeChildren(
                    IDictionary<string, StaticNodeAssetNode> newChildren,
                    IDictionary<string, StaticNodeAssetNode> existing)
                {
                        ...
                        else
                        {
                            if (value.Patterns != null)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        var newList = new StaticNodeAssetPattern[existingNode.Patterns.Length + value.Patterns.Length];
            ...
            public override void Write(Utf8JsonWriter writer, Dictionary<string, StaticNodeAssetNode> value, JsonSerializerOptions options)
            {
