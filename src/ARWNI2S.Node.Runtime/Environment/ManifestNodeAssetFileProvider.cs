...
{
    internal sealed partial class ManifestNodeAssetFileProvider : IFileProvider
    {
        ...
        public ManifestNodeAssetFileProvider(LocalAssetManifest manifest, Func<string, IFileProvider> fileProviderFactory)
        {
        ...
        public IDirectoryContents GetDirectoryContents(string subpath)
        {
            ...
            void GetCandidateFilesForNode(LocalAssetNode candidate, HashSet<IFileInfo> files)
            {
                foreach (var child in candidate.Children!)
                {
                    ...
                    {
                        // This is a folder
                        var file = new LocalAssetsDirectoryInfo(child.Key);
                    ...
                    else
                    {
                        ...
                        files.Add(string.Equals(child.Key, match.Path, _fsComparison) ? file :
                            ...
                            new LocalAssetsFileInfo(child.Key, file));
                    }
                }
            }
        }
        ...
        public IFileInfo GetFileInfo(string subpath)
        {
            ...
            List<LocalAssetPattern> patterns = null;
            ...
            if (match != null)
            {
                ...
                {
                    return new LocalAssetsFileInfo(segments[^1], file);
                }
            }
        ...
        private sealed class LocalAssetsDirectoryContents : IDirectoryContents
        {
            private readonly IEnumerable<IFileInfo> _files;

            public LocalAssetsDirectoryContents(IEnumerable<IFileInfo> files) =>
                _files = files;
        ...
        private sealed class LocalAssetsDirectoryInfo : IFileInfo
        {
            private static readonly DateTimeOffset _lastModified = DateTimeOffset.FromUnixTimeSeconds(0);

            public LocalAssetsDirectoryInfo(string name)
            {
                Name = name;
            }
        ...
        private sealed class LocalAssetsFileInfo : IFileInfo
        {
            private readonly IFileInfo _source;

            public LocalAssetsFileInfo(string name, IFileInfo source)
            {
                Name = name;
                _source = source;
            }
        ...
        internal sealed class LocalAssetManifest
        {
            ...
            internal static LocalAssetManifest Parse(Stream manifest)
            {
                return JsonSerializer.Deserialize(
                    manifest,
                    SourceGenerationContext.DefaultWithConverter.LocalAssetManifest)!;
            }
        }

        [JsonSourceGenerationOptions]
        [JsonSerializable(typeof(LocalAssetManifest))]
        ...
        internal sealed class LocalAssetMatch
        {
        ...
        internal sealed class LocalAssetPattern
        {
            [JsonPropertyName("ContentRootIndex")]
            public int ContentRoot { get; set; }
        ...
        private sealed class OSBasedCaseConverter : JsonConverter<Dictionary<string, LocalAssetNode>>
        {
            public override Dictionary<string, LocalAssetNode> Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
            {
                ...
                var result = new Dictionary<string, LocalAssetNode>(LocalAssetManifest.PathComparer);
                        ...
                        else
                        {
                            if (value.Patterns != null)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        var newList = new LocalAssetPattern[existingNode.Patterns.Length + value.Patterns.Length];
