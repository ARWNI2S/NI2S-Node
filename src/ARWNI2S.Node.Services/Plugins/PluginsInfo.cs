...
namespace ARWNI2S.Node.Services.Plugins
{
    ...
    public partial class PluginsInfo : IPluginsInfo
    {
        ...
        public PluginsInfo(IEngineFileProvider fileProvider)
        {
        ...
        public virtual void LoadPluginInfo()
        {
            ...
            foreach (var item in GetDescriptionFilesAndDescriptors(pluginsDirectory))
            {
                ...
                if (string.IsNullOrEmpty(pluginDescriptor.SystemName?.Trim()))
                    throw new NodeException($"A plugin '{descriptionFile}' has no system name. Try assigning the plugin a unique name and recompiling.");

                if (pluginDescriptors.Any(p => p.pluginDescriptor.Equals(pluginDescriptor)))
                    throw new NodeException($"A plugin with '{pluginDescriptor.SystemName}' system name is already defined");
                ...
                catch (Exception exception)
                {
                    ...
                    throw new NodeException($"Plugin '{pluginDescriptor.FriendlyName}'. {exception.Message}", exception);
                }
            }
